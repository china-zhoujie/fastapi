...
@app03.get('/enum/{city}')
async def latest(city: CityName): #枚举类型的参数
    if city == CityName.Shanghai:
        return {"city_name":city,"confirmed":1587,"death":140}
    elif city == CityName.Beijing:
        return {"city_name":city,"confirmed":1103,"death":90}
    else:
        return {"city_name":city,"latest":unknown}
    
@app03.get('/files/{file_path:path}')
async def filepath(file_path: str):# 通过path parameters传递文件路径
    return f"The filepath is {file_path}"
    
@app03.get('/path_/{num}')
async def path_params_validata(num: int = Path(...,title="Your name",descrition="不可描述",ge=1,le=10)):
    return num
...
@app03.post("/request_body/city1")
def city_info(city: CityInfo): #将请求体的值赋予city
...
@app03.get('/header')
async def header(user_agent: Optional[str] = Header(None,covent_underscores=True),x_token:List[str] = Header(None)):
    """
