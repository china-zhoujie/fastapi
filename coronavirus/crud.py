...
def get_city(db: Session, city_id: int = None):
    if city_id is None:
...
def get_data(db: Session, city: str = None, skip: int = 0, limit: int = 10):
    if city:
        return db.query(models.Data).filter(models.Data.city.has(province=city)).offset(skip).limit(limit).all()  # 外键关联查询，这里不是像Django ORM那样Data.city.province
